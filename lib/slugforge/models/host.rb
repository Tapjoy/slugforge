...
module Slugforge
  class Host
    ...
    def copy_slug(ssh, slug_name, logger, opts)
      ...
      else # wget by default
        logger.log "copying slug to host via wget (#{name})", {:color => :green, :status => :copy, :log_level => :verbose}
        @deploy_results << ssh_command(ssh, "sudo wget -q '#{opts[:url]}' -O #{slug_name_with_path} ; echo \"SSH_COMMAND_EXIT_CODE=$?\"", logger)
      end
      record_event :copied
      slug_name_with_path
    end
    ...
    def install_slug(ssh, slug_name_with_path, logger, opts)
      ...
      # This is a hack to hide that TIAB has both tapjoyserver and tapjoyserver-masterjobs.
      # If this deploy is tapjoyserver, and the box also has tapjoyserver-masterjobs,
      # assume that we need to install both.
      if opts[:masterjobs]
        @deploy_results << ssh_command(ssh, slug_install_command(slug_name_with_path, "#{opts[:deploy_dir]}-masterjobs", {:owner => opts[:owner], :env => opts[:env], :prefix => "[ -e /opt/apps/tapjoyserver-masterjobs ] && "}), logger)
      end
