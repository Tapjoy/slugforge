...
module Slugforge
  module Commands
    class Deploy < Slugforge::SubCommand
      ...
      class_option :'no-masterjobs', :type => :boolean, :hide => true,
        :desc => "Don't magically install masterjobs with tapjoyserver on TIAB"
      ...
      # AWS will throttle wget at 100 connections to a single file
      ...
      def name(name_part, *hosts)
        ...
        deploy(hosts, slug_name, deploy_options(:copy_type => :wget, :url => url))
      end
      ...
      def deploy_options(opts={})
        {
          ...
          :force      => force?,
          :pretend    => pretend?,
          :masterjobs => project_name == 'tapjoyserver' && !options[:'no-masterjobs']
        }.merge(opts)
      end
      ...
      def confirm_deployment_start?(host_groups)
        ...
        if !force? && (ask("Are you sure you wish to deploy? [yN]").downcase != 'y')
